...
# ... (format_shareholders 関数は変更なし。そのまま残してください) ...
...
def extract_data(code, raw_data):
    ...
    if loan == 0:
        ...
        if loan == 0: 
            t_debt = get_fin_value(bs, "Total Debt")
            if t_debt != 0:
                loan = t_debt
    ...
    if total_equity and total_equity != 0:
        debt_equity_ratio = (loan / total_equity) * 100

    loan_equity_ratio = debt_equity_ratio
    ...
    result = {
        ...
        # ★★★ 変更箇所: キー名を Owners -> Shareholders に変更 ★★★
        ...
        "Non-Current Loan": non_current_loan,
        "Loan": loan,
...
def format_for_excel(df):
    ...
    money_cols = [
        ...
        'Current Loan', 'Non-Current Loan', 'Loan'
    ]
    divisor = 1000000.0
    ...
    for col in money_cols:
        if col in df.columns:
            # 数値(float)に変換して計算
            # 文字列置換(replace)は行わず、純粋な数値として保持する -> Excelで右揃えになる
    ...
    # ★★★ 修正箇所: 日付を "YYYY/MM/DD" 形式の【文字列】に変換する ★★★
    print("日付を YYYY/MM/DD 形式の文字列に変換しています...")
    date_cols = ['FY']
    for col in date_cols:
        if col in df.columns:
            # 日付型に変換してから、指定フォーマットの文字列にする (NaTは空文字へ)
